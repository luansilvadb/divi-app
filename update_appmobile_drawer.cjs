...
const content = `<template>
  <Drawer
    ...
    :pt="{
      ...
      header: { class: 'p-4 border-b border-black/5 dark:border-white/5 shrink-0 flex items-center justify-between' },
      title: { class: 'font-bold text-lg text-text-primary' },
      closeButton: { class: 'p-2 -mr-2 text-text-secondary hover:text-text-primary rounded-full hover:bg-black/5 dark:hover:bg-white/5 transition-colors border-none bg-transparent cursor-pointer' },
  ...
  >
    ...
    <div class="p-4 border-t border-black/5 dark:border-white/5 flex items-center justify-between pb-safe mt-auto">
      ...
      <button
        @click="handleLogout"
        class="flex items-center gap-3 px-3 py-3 rounded-xl text-error-main hover:bg-error-main/10 transition-colors cursor-pointer border-none bg-transparent"
      >
