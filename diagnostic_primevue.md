...
*   **Boilerplate em Lógica Visual:** A implementação manual de diálogos (`BaseModal`) e selects exigia o uso de diretivas customizadas (`v-click-outside`) e lógica de gerenciamento de estado (`isOpen`) que aumentavam a carga cognitiva e o tamanho do bundle, ferindo o princípio de Densidade Lógica.
...
*   **Refatoração de Diálogos e Acessibilidade:** O `BaseModal` foi reconstruído utilizando o componente `Dialog` do PrimeVue. Isso resolveu nativamente problemas de focus trap, bloqueio de scroll do body e suporte a leitores de tela, mantendo a regra de plataforma (`isMobile` para troca entre Dialog e BottomSheet).
...
*   **Otimização de Performance:** A migração para componentes PrimeVue, que utilizam internamente padrões de performance otimizados para Vue 3, combinada com o uso de `shallowRef` para grandes volumes de dados (conforme diagnosticado anteriormente), resultou em uma interface mais fluida e responsiva.
