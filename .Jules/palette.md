...
## 2026-03-31 - [Acessibilidade em Itens de Lista Interativos] **Aprendizado:** [Elementos `div` interativos (como itens de lista de transações) que agem como botões e contêm ações secundárias ocultas precisam de `role="button"`, `tabindex="0"` e listeners para `@keydown.enter` e `@keydown.space`. Além disso, ações ocultas baseadas em `group-hover` devem incluir `group-focus-within` para serem reveladas e acessadas via teclado.] **Ação:** [Sempre aplicar `role="button"`, `tabindex="0"`, eventos de teclado adequados e estados de `focus-within` para ações ocultas em elementos `div` que funcionam como itens de lista clicáveis, garantindo navegação completa por teclado.]

## 2024-05-19 - Melhoria no foco via teclado na AppSidebar **Aprendizado:** Os botões e links customizados da sidebar não tinham uma indicação visual de foco, dificultando a navegação via teclado, quebrando diretrizes de acessibilidade (WCAG). **Ação:** Sempre definir os estados `:focus-visible` em componentes customizados que não usam as classes utilitárias base do framework, garantindo o contraste necessário.

## 2026-03-31 - Atributos ARIA em Botões de Loading
**Aprendizado:** Identifiquei que botões que possuem um estado de loading com um spinner não comunicam visualmente sua mudança de estado para tecnologias assistivas. Além de botões ícones e inputs precisarem de `aria-label`, os botões em loading precisam notificar seu estado.
**Ação:** Adicionar sempre os atributos `aria-busy` dinâmicos a componentes em operação assíncrona, e usar `aria-hidden` para esconder ícones (como svg e div spinners) nesses elementos, focando sempre na experiência via teclado com pseudo-classes `focus-visible`.
