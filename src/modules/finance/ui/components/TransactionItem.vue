<template>
  <div class="transaction-item flex items-center p-4 gap-4 cursor-pointer relative transition-all duration-300 border-b border-black/5 dark:border-white/5 last:border-0 hover:bg-black/2 dark:hover:bg-white/2 group" @click="$emit('click')">
    <div class="item-left flex items-center gap-4 flex-1 min-w-0">
      ...
      <div class="t-main min-w-0 flex flex-col gap-0.5">
        <div class="t-title-row flex items-center gap-2">
          <span class="t-title font-bold text-text-primary text-sm truncate tracking-tight">{{ transaction.title }}</span>
          <BaseBadge v-if="walletName" status="normal" variant="subtle" class="scale-75 origin-left">
            {{ walletName }}
          </BaseBadge>
        </div>
        <div class="t-details-row flex items-center gap-2">
          ...
          <span v-if="showTime" class="text-[0.65rem] font-bold text-text-disabled uppercase tracking-widest">
    ...
    <div class="t-meta flex items-center gap-4">
      <span :class="['font-black text-sm tracking-tight', transaction.type === 'expense' ? 'text-text-primary' : 'text-success-main']">
...
const iconColor = computed(() => {
  return props.transaction.type === 'income' ? 'var(--color-success-main)' : 'var(--color-error-main)'
})
...
<style scoped>
/* Scoped styles replaced by Tailwind and Global CSS */
</style>
