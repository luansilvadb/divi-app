      ...
      <template #action>
        ...
        <div class="flex items-center bg-bg-main dark:bg-black/20 p-1.5 rounded-2xl border border-black/5 dark:border-white/5">
          ...
          <div class="px-6 flex flex-col items-center min-w-[140px]">
            ...
            <span class="text-sm font-black text-text-primary tracking-tight leading-none uppercase">{{ monthName }}</span>
          </div>
        ...
        <div class="glass-card p-6 shadow-sm overflow-hidden">
          ...
          <div class="grid grid-cols-7 gap-3">
            <div 
              ...
              :class="[
                day.isCurrentMonth ? 'bg-white/40 dark:bg-black/10 hover:bg-white dark:hover:bg-white/10 hover:border-black/5 dark:hover:border-white/10' : 'opacity-20 cursor-default',
                isSelected(day.date) ? 'ring-2 ring-primary-main/40 border-primary-main/20 bg-white/80 dark:bg-white/5' : ''
              ]"
            ...
            >
              <span 
                ...
                :class="isToday(day.date) ? 'bg-primary-main text-white shadow-lg shadow-primary-main/20' : 'text-text-primary'"
              >
                {{ day.dayNumber }}
              </span>

              <!-- Event Dots -->
              <div class="mt-auto flex gap-1 flex-wrap justify-center max-w-full overflow-hidden pb-1">
                <div 
                ...
                ></div>
                <span v-if="getTransactionsForDate(day.date).length > 3" class="text-[8px] font-black text-text-disabled">
      ...
      <aside class="side-column">
        <BaseCard>
          <template #header>
            <div class="flex flex-col gap-1">
              ...
              <span class="text-[10px] font-black uppercase tracking-widest text-text-disabled">{{ formatDateFull(selectedDate) }}</span>
            </div>
          </template>

          <div v-if="selectedDateTransactions.length === 0" class="flex flex-col items-center justify-center py-16 opacity-40 text-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="mb-4"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
          ...
          <ul v-else class="list-none p-0 m-0 flex flex-col gap-2 pt-2">
            <li v-for="t in selectedDateTransactions" :key="t.id || t.localId" class="flex items-center gap-4 p-3 rounded-xl hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
              <BaseIconBox 
                :color="t.type === 'expense' ? 'var(--color-error-main)' : 'var(--color-success-main)'"
                size="sm"
              >
                <svg v-if="t.type === 'expense'" xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V5M5 12l7-7 7 7"/></svg>
              </BaseIconBox>
              <div class="flex-1 min-w-0">
                <div class="font-bold text-sm text-text-primary truncate">{{ t.title }}</div>
                <div class="text-[10px] font-black uppercase tracking-widest text-text-disabled">{{ t.category_id || 'Geral' }}</div>
              </div>
              <span class="font-black text-sm tracking-tight" :class="t.type === 'expense' ? 'text-error-main' : 'text-success-main'">
        ...
        <BaseCard v-if="selectedDateTransactions.length > 0">
           ...
           <div class="flex flex-col gap-6 pt-2">
             ...
             <div class="w-full p-4 rounded-2xl bg-bg-main dark:bg-black/20 flex flex-col items-center text-center">
               <span class="text-[10px] font-black uppercase tracking-widest text-text-disabled mb-1">Saldo do Dia</span>
               <div class="text-2xl font-black tracking-tight" :class="selectedDateBalance >= 0 ? 'text-primary-main' : 'text-error-main'">
...
const selectedDateIncome = computed(() => selectedDateTransactions.value.filter(t => t.type === 'income').reduce((sum, t) => sum + t.amount, 0))
const selectedDateExpense = computed(() => selectedDateTransactions.value.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.amount, 0))
