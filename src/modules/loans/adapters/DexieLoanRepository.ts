...
export class DexieLoanRepository implements ILoanRepository {
  async getAll(): Promise<ILoan[]> {
    try {
      const loans = await db.loans.filter((l) => !l.deleted).toArray()
      return loans.map(this.mapToEntity)
    } catch (err) {
  ...
  async save(loan: ILoan): Promise<void> {
    try {
      ...
      console.debug('[DexieLoanRepository] Empréstimo salvo localmente de forma atômica.')
    } catch (err) {
  ...
  async delete(id: string): Promise<void> {
    try {
      ...
      SyncEngine.getInstance().enqueueSync()
      console.debug('[DexieLoanRepository] Empréstimo marcado para deleção de forma atômica.')
    } catch (err) {
      throw new InfrastructureError(
