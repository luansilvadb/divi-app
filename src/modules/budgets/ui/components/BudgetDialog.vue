  ...
  <template v-if="isMobile">
    <Drawer
      ...
      :pt="{
        ...
        mask: { class: '!z-[200] bg-black/60 backdrop-blur-sm transition-all duration-300' },
        header: { class: '!p-0' },
...
const onTouchStartHeader = (e: TouchEvent) => {
...
const onTouchStartContent = (e: TouchEvent) => {
...
const onTouchEnd = () => {
  ...
  if (deltaY > 150 || velocity > 0.5) {
    ...
    swipeOffset.value = window.innerHeight
    setTimeout(() => {
      emit('close')
      setTimeout(() => {
        swipeOffset.value = 0
