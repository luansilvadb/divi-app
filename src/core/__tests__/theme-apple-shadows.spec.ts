...
describe('Apple Style Shadows', () => {
  it('should define Apple Low Shadow for light mode', () => {
    const shadow = getComputedStyle(document.documentElement).getPropertyValue('--apple-shadow-low').trim()
    expect(shadow).toContain('rgba(0, 0, 0, 0.1)')
  })

  it('should define Apple Medium Shadow for light mode', () => {
    const shadow = getComputedStyle(document.documentElement).getPropertyValue('--apple-shadow-medium').trim()
