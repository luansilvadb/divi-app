...
export class AssetLoader implements IAssetLoader {
  ...
  sanitize(url: string | undefined): string {
    ...
    // Check protocol
    const isAllowed = ASSET_LIMITS.ALLOWED_PROTOCOLS.some(protocol => cleanUrl.startsWith(protocol));

    if (!isAllowed) {
      this.logWarning('Invalid asset protocol', `Asset URL blocked: ${cleanUrl.substring(0, 50)}...`);
