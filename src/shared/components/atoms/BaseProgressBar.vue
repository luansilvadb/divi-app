<template>
  <div class="w-full">
    ...
    <div
      class="h-2.5 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden relative shadow-inner"
    >
      <!-- Progress indicator (thumb) -->
      <div
        class="h-full transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] rounded-full relative"
        :class="[{ 'animate-pulse-error': isOverBudget || status === 'error' }, statusClass]"
        :style="customStyle"
      >
        <!-- Shimmer effect -->
        <div
