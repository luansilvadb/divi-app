  ...
  >
    ...
    <div class="sidebar-header">
      <RouterLink to="/" class="sidebar-brand group/logo">
        <div class="sidebar-logo-icon transition-transform duration-500 ease-out group-hover/logo:scale-105 group-hover/logo:rotate-3">
          <svg
            width="34"
            height="34"
            viewBox="0 0 44 44"
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden pt-2 pb-6 custom-scrollbar">
      <Menu :model="menuItems" class="border-none bg-transparent w-full">
          <template #item="{ item, props }">
              <RouterLink v-if="item.route" v-slot="{ href, navigate, isActive }" :to="item.route" custom>
                  <a :href="href" v-bind="props.action" @click="navigate" :class="[
                    ...
                    isActive ? 'bg-primary/10 text-primary font-bold shadow-sm' : 'text-surface-600 dark:text-surface-200 hover:bg-surface-50 dark:hover:bg-surface-800/10'
                  ]">
          ...
          <template #submenuheader="{ item }">
              <div class="px-6 py-2 text-[0.65rem] font-black uppercase tracking-[0.2em] text-surface-400/50 dark:text-surface-400/50 mt-4 mb-2" :class="{ 'opacity-0 h-0 p-0 hidden': isCollapsed }">
                  {{ item.label }}
              </div>
          </template>
      </Menu>
    ...
    <div class="sidebar-footer mt-auto flex flex-col pt-4 border-t border-surface-200 dark:border-surface-800/10">
      ...
      >
        ...
        <div class="relative flex-none">
          <div class="w-10 h-10 rounded-full bg-surface-0 dark:bg-surface-800 flex items-center justify-center overflow-hidden border border-black/10 dark:border-white/10 shadow-sm transition-transform duration-500 group-hover/user:scale-110">
        ...
        <div 
          ...
          :class="{ 'opacity-0 w-0 hidden': isCollapsed }"
        >
          <div class="text-[0.8rem] font-bold text-surface-800 dark:text-surface-50 truncate tracking-tight text-left">Luan Silva</div>
          <div class="text-[0.6rem] font-medium text-surface-600/40 dark:text-surface-200/40 truncate tracking-tight uppercase text-left">luan@divi.app</div>
        </div>
