<template>
  <div class="fixed bottom-6 left-0 right-0 z-[100] px-4 pointer-events-none md:hidden will-change-transform">
    <nav class="pointer-events-auto mx-auto w-full max-w-[440px] bg-surface-0/90 dark:bg-surface-900/90 backdrop-blur-md border border-surface-200/30 dark:border-surface-800/50 shadow-lg rounded-[2.5rem] p-1 flex items-center ring-1 ring-black/5 dark:ring-white/5 overflow-hidden">
      <template v-for="item in items" :key="item.label">
        <RouterLink v-if="item.route" v-slot="{ href, navigate, isActive }" :to="item.route" custom>
          <a :href="href" @click="navigate" :class="[
            ...
            isActive ? 'text-primary' : 'text-surface-600 dark:text-surface-400 font-medium'
          ]">
            ...
            <i 
              :class="[item.icon, { 'scale-110': isActive }]" 
              class="text-[1.2rem] transition-transform duration-200 relative z-10"
            ></i>
            
            <span 
              :class="['text-[8px] font-black uppercase tracking-widest transition-opacity duration-200 leading-none relative z-10', isActive ? 'opacity-100' : 'opacity-60']"
            >
              {{ item.label }}
            </span>
