<template>
  ...
  >
    <nav
      class="pointer-events-auto mx-auto w-full max-w-[440px] bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-800/50 shadow-2xl rounded-[2.5rem] p-1 flex items-center ring-1 ring-black/5 dark:ring-white/5 overflow-hidden"
    >
        ...
        <RouterLink v-if="item.route" v-slot="{ href, navigate }" :to="item.route" custom>
          <a
            ...
            :class="[
              ...
              isPageActive(item.route) ? 'text-violet-500' : 'text-zinc-500 dark:text-zinc-400 font-medium',
            ]"
          >
            <div
              v-if="isPageActive(item.route)"
              class="absolute inset-0 bg-violet-500/5 rounded-[1.5rem] transition-opacity duration-300 pointer-events-none"
            ></div>
            ...
            <div v-if="isPageActive(item.route)" class="absolute bottom-1 w-1 h-1 bg-violet-500 rounded-full"></div>
          </a>
        </RouterLink>
