<template>
  <div class="w-full">
    ...
    <div class="bg-primary-main/10 p-7 border-b border-white/5 relative">
      ...
      <div class="flex items-center justify-between relative z-10">
        <div class="flex items-center gap-4">
          <div
            class="w-12 h-12 rounded-2xl bg-primary-main/20 flex items-center justify-center text-primary-main shadow-inner border border-primary-main/20 shrink-0"
          >
            <svg
          ...
          <div>
            <h2 class="text-xl sm:text-2xl font-black text-text-primary tracking-tight leading-tight">
              Nova Transação
            </h2>
            <p
              class="text-[0.6rem] sm:text-[0.65rem] font-black uppercase tracking-[0.2em] text-text-secondary opacity-40"
            >
              Adicione um novo lançamento
            </p>
          </div>
        </div>
        <button
          @click="$emit('close')"
          class="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-white/10 transition-all border border-white/5 group shrink-0"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="20"
    ...
    <!-- Body Area -->
    <form @submit.prevent="handleSubmit" class="p-5 sm:p-8 space-y-6 sm:space-y-8 bg-black/10">
      <!-- Type Switcher (Premium) -->
      <div class="flex p-1.5 bg-white/5 rounded-2xl gap-1.5 border border-white/5 shadow-inner">
        <button
          type="button"
          class="flex-1 py-3.5 px-2 sm:px-4 rounded-xl font-black text-[0.65rem] sm:text-[0.7rem] uppercase tracking-[0.1em] sm:tracking-[0.15em] transition-all flex items-center justify-center gap-2 sm:gap-3 group relative overflow-hidden"
          :class="
            form.type === 'expense'
      ...
      <!-- Action Buttons -->
      <div class="flex items-center gap-3 sm:gap-4 pt-2 sm:pt-4 pb-2">
        <BaseButton
          variant="ghost"
          type="button"
          class="flex-1 !py-3.5 sm:!py-4 font-black uppercase text-[0.65rem] sm:text-[0.7rem] tracking-widest opacity-40 hover:opacity-100 hover:bg-white/5 transition-all"
          @click="$emit('close')"
        >
          Cancelar
        </BaseButton>
        <BaseButton
          ...
          class="flex-[2] !py-3.5 sm:!py-4 font-black uppercase text-[0.65rem] sm:text-[0.7rem] tracking-[0.1em] sm:tracking-[0.15em] shadow-2xl transition-all hover:scale-[1.02] active:scale-[0.98]"
