<template>
  <BottomSheet
    ...
    teleport-to="body"
    :snap-points="['99%', instinctHeight]"
    @instinct-height="(n) => (instinctHeight = n)"
  >
    <template #header>
      <!-- Visual Drag Handle -->
      <div class="flex justify-center pt-3 pb-1 bg-surface-main dark:bg-surface-main rounded-t-[1.5rem]">
        <div class="w-12 h-1.5 rounded-full bg-black/10 dark:bg-white/10"></div>
      </div>

      <!-- Header Area (Premium Split) -->
      <div
        class="bg-primary-main/10 p-5 border-b border-black/5 dark:border-white/5 relative bg-surface-main dark:bg-surface-main text-text-primary"
      >
        <div
