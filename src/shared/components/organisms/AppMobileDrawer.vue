<template>
  ...
  >
    ...
    >
      ...
      <div class="relative z-10 flex items-center justify-between px-6 py-5">
        <div class="flex flex-col">
          <span
            class="text-xl font-black text-surface-800 dark:text-surface-50 tracking-tighter leading-none mb-1"
            >Mais Opções</span
          >
          <div class="flex items-center opacity-50">
            <span
              class="text-[0.6rem] font-bold uppercase tracking-[0.15em] text-surface-600 dark:text-surface-200"
              >EXPLORE O SISTEMA</span
            >
          </div>
        </div>
      ...
      <Menu :model="menuItems" class="border-none bg-transparent w-full">
            ...
            <a
              ...
              :class="[
                ...
                isActive
                  ...
                  : 'text-surface-600 dark:text-surface-400 hover:bg-surface-50 dark:hover:bg-surface-800/20',
              ]"
            >
              <span
                :class="[
                  item.icon,
                  isActive ? 'scale-110 shadow-primary/20' : 'opacity-70 group-hover:opacity-100',
                ]"
                class="w-6 h-6 flex items-center justify-center text-lg transition-transform"
              ></span>
              <span class="font-black text-[0.85rem] uppercase tracking-widest">{{
                item.label
              }}</span>
              ...
              <div
                v-if="isActive"
                class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-primary rounded-r-full shadow-[0_0_12px_rgba(var(--primary-color-rgb),0.5)]"
              ></div>
            </a>
          </RouterLink>
        </template>
        <template #submenuheader="{ item }">
          <div class="px-4 mb-3 mt-8 first:mt-0">
            <span
              class="text-[0.6rem] font-black uppercase tracking-[0.25em] text-surface-400 dark:text-surface-500/60"
            >
              {{ item.label }}
            </span>
    ...
    <!-- Footer Actions -->
    <div
      class="p-6 border-t border-surface-200 dark:border-surface-800/10 flex flex-col gap-4 pb-safe mt-auto"
    >
      <div class="flex items-center justify-between">
        <span class="text-[0.65rem] font-black uppercase tracking-widest text-surface-400"
          >Aparência</span
        >
        <ThemeToggle />
      </div>
